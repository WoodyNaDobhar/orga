...
class MeetupAPIController extends AppBaseController
{
	 ...
	 * 		createdBy (User) (BelongsTo): Meetup that created it.
	 * 		updatedBy (User) (BelongsTo): Meetup that last updated it (if any).
	 * 		deletedBy (User) (BelongsTo): Meetup that deleted it (if any).",
	 *		@OA\Parameter(
	...
	public function index(Request $request): JsonResponse
	{
		try {
			...
			$this->authorize('viewAny', Meetup::class);
			...
			return $this->sendResponse(new MeetupResource($meetups), 'Meetups retrieved successfully.');
		} catch (Throwable $e) {
	...
	/**
	 * @param CreateMeetupAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/meetups",
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			/** @var Meetup $meetup */
			$meetup = $this->meetupRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
	...
	/**
	 * @param int $id
	 * @param UpdateMeetupAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/meetups/{id}",
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($meetup)) {
				return $this->sendError('Meetup (' . $id . ') not found.', ['id' => $id], 404);
			}
