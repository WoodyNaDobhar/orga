...
// 			$this->authorize('viewAny', Recommendation::class);
	...
	/**
	 * @param CreateRecommendationAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/recommendations",
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			...
			$recommendation = $this->recommendationRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
...
// 			$this->authorize('view', $recommendation);
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($recommendation)) {
				return $this->sendError('Recommendation (' . $id . ') not found.', ['id' => $id], 404);
			}
