...
class OfficerAPIController extends AppBaseController
{
	 ...
	 * 		createdBy (User) (BelongsTo): Officer that created it.
	 * 		updatedBy (User) (BelongsTo): Officer that last updated it (if any).
	 * 		deletedBy (User) (BelongsTo): Officer that deleted it (if any).",
	 *		@OA\Parameter(
	...
	public function index(Request $request): JsonResponse
	{
		try {
			...
			return $this->sendResponse(new OfficerResource($officers), 'Officers retrieved successfully.');
		} catch (Throwable $e) {
	...
	/**
	 * @param CreateOfficerAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/officers",
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			/** @var Officer $officer */
			$officer = $this->officerRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
	...
	/**
	 * @param int $id
	 * @param UpdateOfficerAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/officers/{id}",
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($officer)) {
				return $this->sendError('Officer (' . $id . ') not found.', ['id' => $id], 404);
			}
