...
// 			$this->authorize('viewAny', Chaptertype::class);
	...
	/**
	 * @param CreateChaptertypeAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/chaptertypes",
	...
	public function store(CreateChaptertypeAPIRequest $request): JsonResponse
	{
		try {
			...
			$this->authorize('create', Chaptertype::class);
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			...
			$chaptertype = $this->chaptertypeRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
...
// 			$this->authorize('view', $chaptertype);
	...
	/**
	 * @param int $id
	 * @param UpdateChaptertypeAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/chaptertypes/{id}",
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($chaptertype)) {
				return $this->sendError('Chaptertype (' . $id . ') not found.', ['id' => $id], 404);
			}
		
			$this->authorize('delete', $chaptertype);
