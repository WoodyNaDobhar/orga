...
class SplitAPIController extends AppBaseController
{
	...
	public function index(Request $request): JsonResponse
	{
		try {
			...
			$this->authorize('viewAny', Split::class);
	...
	/**
	 * @param CreateSplitAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/splits",
	...
	public function store(CreateSplitAPIRequest $request): JsonResponse
	{
		try {
			...
			$this->authorize('create', Split::class);
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			/** @var Split $split */
			$split = $this->splitRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
	...
	/**
	 * @param int $id
	 * @param UpdateSplitAPIRequest $request
	 * @return Response
	 *
	 * @OA\Post(
	 *		path="/splits/{id}",
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($split)) {
				return $this->sendError('Split (' . $id . ') not found.', ['id' => $id], 404);
			}
