...
class AttendanceAPIController extends AppBaseController
{
	 ...
	 * 		createdBy (User) (BelongsTo): Attendance that created it.
	 * 		updatedBy (User) (BelongsTo): Attendance that last updated it (if any).
	 * 		deletedBy (User) (BelongsTo): Attendance that deleted it (if any).",
	 *		@OA\Parameter(
	...
	public function index(Request $request): JsonResponse
	{
		try {
			...
			return $this->sendResponse(new AttendanceResource($attendances), 'Attendances retrieved successfully.');
		} catch (Throwable $e) {
	...
	public function show($id, Request $request): JsonResponse
	{
		try {
			...
			$attendance = $this->attendanceRepository->find(
				$id,
				$request->has('columns') ? $request->get('columns') : ['*'],
				$request->has('with') ? $request->get('with') : null
			);
	...
	public function destroy($id): JsonResponse
	{
		try {
			...
			if (empty($attendance)) {
				return $this->sendError('Attendance (' . $id . ') not found.', ['id' => $id], 404);
			}
